...
pycircdb is a CLI tool to annotate circular RNAs output by circRNA quantification tools, integrating multiple public circRNA databases into a single, reproducible set of outputs across many samples.
...
pycircdb is designed to bring traditional GUI circRNA databases to the CLI, facilitating its use in computational workflows.  
pycircdb is lightweight - 4.2 GB cached database size and users can share the cached directory to reduce disk space across multiple runs. 
