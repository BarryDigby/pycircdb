...
"""
Code to initilise the pycircdb logging
Credit @ewels MultiQC
"""
...
from pycircdb.utils import config, util_functions
...
def init_log(logger, loglevel=0, no_ansi=False):
    ...
    Also prints verbose log to the pycircdb data directory if available.
    (pycircdb_results/pycircdb.log)
    ...
    log_tmp_fn = os.path.join(log_tmp_dir, "pycircdb.log")
...
def move_tmp_log(logger):
    ...
    try:
        ...
        logging.shutdown()
        shutil.copy(log_tmp_fn, os.path.join(config.data_dir, "pycircdb.log"))
